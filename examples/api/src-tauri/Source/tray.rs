...
pub fn create_tray<R:Runtime>(app:&tauri::AppHandle<R>) -> tauri::Result<()> {
	...
	let menu2 =
		Menu::with_items(app, &[&toggle_i, &new_window_i, &switch_i, &quit_i, &remove_tray_i])?;
	...
	let _ = TrayIconBuilder::with_id("tray-1")
		...
		.on_menu_event(move |app, event| {
			match event.id.as_ref() {
				...
				"new-window" => {
					let _ =
						WebviewWindowBuilder::new(app, "new", WebviewUrl::App("index.html".into()))
							.title("Tauri")
							.build();
				},
				#[cfg(target_os = "macos")]
				...
				"switch-menu" => {
					...
					let (menu, tooltip) =
						if flag { (menu2.clone(), "Menu 2") } else { (menu1.clone(), "Tauri") };
					if let Some(tray) = app.tray_by_id("tray-1") {
