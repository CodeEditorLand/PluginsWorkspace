...
pub fn run() {
		...
		.setup(move |app| {
			...
			#[cfg(target_os = "windows")]
			{
				webview_window_builder =
					webview_window_builder.transparent(true).shadow(true).decorations(false);
			}
		...
		.on_page_load(|webview, payload| {
			if payload.event() == PageLoadEvent::Finished {
				...
				webview.listen("js-event", move |event| {
					...
					webview_.emit("rust-event", Some(reply)).expect("failed to emit");
				});
			}
		});
	...
	#[allow(unused_mut)]
	let mut app = builder
		.invoke_handler(tauri::generate_handler![cmd::log_operation, cmd::perform_request,])
