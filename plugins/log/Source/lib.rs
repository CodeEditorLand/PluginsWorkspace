	...
	impl NSString {
		pub fn new(s:&str) -> Self {
			...
			NSString(unsafe {
				...
				// autorelease in rust to relinquish control or it can not
				// be released correctly in OC runtime
...
pub enum TargetKind {
	...
	Folder { path:PathBuf, file_name:Option<String> },
...
impl Builder {
	...
	/// Removes all targets. Useful to ignore the default targets and
	/// reconfigure them.
	...
	/// tauri_plugin_log::Builder::new().target(Target::new(TargetKind::Webview));
	/// ```
	...
	/// tauri_plugin_log::Builder::new().clear_targets().targets([
