...
impl<R:Runtime> Opener<R> {
	...
	/// - **Android / iOS**: Always opens using default program.
	#[cfg(desktop)]
	pub fn open_path(&self, path:impl Into<String>, with:Option<impl Into<String>>) -> Result<()> {
	...
	/// - **Android / iOS**: Always opens using default program.
	#[cfg(mobile)]
	pub fn open_path(&self, path:impl Into<String>, _with:Option<impl Into<String>>) -> Result<()> {
		self.mobile_plugin_handle
...
impl<R:Runtime, T:Manager<R>> OpenerExt<R> for T {
	fn opener(&self) -> &Opener<R> { self.state::<Opener<R>>().inner() }
}
...
impl Default for Builder {
	fn default() -> Self { Self { open_js_links_on_click:true } }
}
...
impl Builder {
	...
	/// Whether the plugin should inject a JS script to open URLs in default
	/// browser when clicking on `<a>` elements that has `_blank` target, or
	/// when pressing `Ctrl` or `Shift` while clicking it.
	///
