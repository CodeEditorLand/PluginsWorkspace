...
fn _f() {
	match (scope::EntryRaw::Url { url:String::new(), app:scope::Application::Enable(true) }) {
		scope::EntryRaw::Url { url, app } => {
			OpenerScopeEntry::Url {
				url,
				app:match app {
		...
		scope::EntryRaw::Path { path, app } => {
			OpenerScopeEntry::Path {
				path,
				app:match app {
					scope::Application::Enable(p) => Application::Enable(p),
					scope::Application::App(p) => Application::App(p),
	...
	match (OpenerScopeEntry::Url { url:String::new(), app:Application::Enable(true) }) {
		OpenerScopeEntry::Url { url, app } => {
			scope::EntryRaw::Url {
				url,
				app:match app {
		...
		OpenerScopeEntry::Path { path, app } => {
			scope::EntryRaw::Path {
				path,
				app:match app {
					Application::Enable(p) => scope::Application::Enable(p),
