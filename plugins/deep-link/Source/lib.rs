...
fn init_deep_link<R:Runtime>(
	...
	#[cfg(target_os = "android")]
	{
		...
		return Ok(DeepLink { app:app.clone(), plugin_handle:handle });
	}
...
mod imp {
	...
	impl<R:Runtime> DeepLink<R> {
		/// Get the current URLs that triggered the deep link. Use this on app
		/// load to check whether your app was started via a deep link.
		///
		/// ## Platform-specific:
		///
		/// - **Windows / Linux**: This function reads the command line
		///   arguments and checks if there's only one value, which must be an
		///   URL with scheme matching one of the configured values. Note that
		///   you must manually check the arguments when registering deep link
		///   schemes dynamically with [`Self::register`]. Additionally, the
		///   deep link might have been provided as a CLI argument so you should
		///   check if its format matches what you expect.
		...
		///
		/// - `protocol`: The name of the protocol without `://`. For example,
		///   if you want your app to handle `tauri://` links, call this method
		///   with `tauri` as the protocol.
		///
		/// ## Platform-specific:
		///
		...
		/// Unregister the app as the default handler for the specified
		/// protocol.
		///
		...
		///   registered with [`register`](`Self::register`). May not work on
		///   older distros.
		/// - **macOS / Android / iOS**: Unsupported, will return
		///   [`Error::UnsupportedPlatform`](`crate::Error::UnsupportedPlatform`).
		...
		/// Check whether the app is the default handler for the specified
		/// protocol.
		///
		/// - `protocol`: The name of the protocol without `://`.
		///
...
mod imp {
	...
	impl<R:Runtime> DeepLink<R> {
		/// Checks if the provided list of arguments (which should match
		/// [`std::env::args`]) contains a deep link argument (for Linux and
		/// Windows).
		///
		/// On Linux and Windows the deep links trigger a new app instance with
		/// the deep link URL as its only argument.
		///
		/// This function does what it can to verify if the argument is actually
		/// a deep link, though it could also be a regular CLI argument.
		/// To enhance its checks, we only match deep links against the schemes
		/// defined in the Tauri configuration i.e. dynamic schemes WON'T be
		/// processed.
		///
		...
		///
		/// This is useful to ensure the schemes are registered even if the user
		/// did not install the app properly (e.g. an AppImage that was not
		/// properly registered with an AppImage launcher).
		...
		pub fn register<S:AsRef<str>>(&self, _protocol:S) -> crate::Result<()> {
			...
			#[cfg(target_os = "linux")]
			{
				...
				Command::new("update-desktop-database").arg(target).status()?;
...
impl<R:Runtime, T:Manager<R>> crate::DeepLinkExt<R> for T {
	fn deep_link(&self) -> &DeepLink<R> { self.state::<DeepLink<R>>().inner() }
}
...
impl OpenUrlEvent {
	/// The event ID which can be used to stop listening to the event via
	/// [`tauri::Listener::unlisten`].
	pub fn id(&self) -> EventId { self.id }

	/// The event URLs.
	pub fn urls(self) -> Vec<Url> { self.urls }
}

impl<R:Runtime> DeepLink<R> {
	/// Helper function for the `deep-link://new-url` event to run a function
	/// each time the protocol is triggered while the app is running.
	///
	/// Use `get_current` on app load to check whether your app was started via
	/// a deep link.
	pub fn on_open_url<F:Fn(OpenUrlEvent) + Send + Sync + 'static>(&self, f:F) -> EventId {
		let event_id = self.app.listen("deep-link://new-url", move |event| {
			if let Ok(urls) = serde_json::from_str(event.payload()) {
				f(OpenUrlEvent { id:event.id(), urls })
			}
		});
