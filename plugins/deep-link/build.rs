...
// TODO: Consider using activity-alias in case users may have multiple
// activities in their app. TODO: Do we want to support the other path* configs
// too?
...
fn main() {
	...
	if let Some(config) = tauri_plugin::plugin_config::<Config>("deep-link") {
		tauri_plugin::mobile::update_android_manifest(
			"DEEP LINK PLUGIN",
			"activity",
			config.mobile.iter().map(intent_filter).collect::<Vec<_>>().join("\n"),
		)
