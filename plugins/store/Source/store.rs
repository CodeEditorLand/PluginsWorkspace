...
impl<R:Runtime> StoreBuilder<R> {
	...
	pub fn new<M:Manager<R>, P:AsRef<Path>>(manager:&M, path:P) -> Self {
		Self {
			...
			// Since Store.path is only exposed to the user in emit calls we may
			// as well simplify it here already.
	...
	///
	/// Note: only works if this store is managed by the plugin (e.g. made using
	/// [`crate::with_store`] or inserted into [`crate::Builder`])
	///
	/// # Examples
	/// ```
	/// tauri::Builder::default()
