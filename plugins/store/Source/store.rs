...
impl<R:Runtime> StoreBuilder<R> {
	...
	///
	/// If a store with the same path has already been loaded its instance is
	/// returned.
	///
	/// # Examples
	/// ```
...
impl<R:Runtime> Store<R> {
	...
	/// Returns true if the store contains no elements.
	pub fn is_empty(&self) -> bool { self.store.lock().unwrap().is_empty() }

	/// Update the store from the on-disk state
	pub fn reload(&self) -> crate::Result<()> { self.store.lock().unwrap().load() }
...
impl<R:Runtime> Drop for Store<R> {
	fn drop(&mut self) { self.apply_pending_auto_save(); }
}
