...
impl<R:Runtime> Builder<R> {
	...
	// ///   .setup(|app| {
	// ///     let store =
	// StoreBuilder::new("store.bin").build(app.handle().clone()); ///     let
	// builder = Builder::default().store(store); ///     Ok(())
	// ///   });
	// /// ```
	...
	// ///     let store =
	// StoreBuilder::new("store.bin").build(app.handle().clone()); ///     let
	// builder = Builder::default().stores([store]); ///     Ok(())
	// ///   });
	// /// ```
	// pub fn stores<T: IntoIterator<Item = Store<R>>>(mut self, stores: T) ->
	// Self {     self.stores = stores
	//         .into_iter()
	...
	// ///   .setup(|app| {
	// ///     let store =
	// StoreBuilder::new("store.bin").build(app.handle().clone()); ///     app.
	// handle().plugin(Builder::default().freeze().build()); ///     Ok(())
	// ///   });
	// /// ```
		...
		plugin::Builder::new("store")
			...
			.on_event(|_app_handle, event| {
				if let RunEvent::Exit = event {
					// let collection =
					// app_handle.state::<StoreCollection<R>>();

					// for store in collection.stores.lock().expect("mutex
					// poisoned").values_mut() {     if let Some(sender) =
					// store.auto_save_debounce_sender.take() {         let
					// _ = sender.send(AutoSaveMessage::Cancel);     }
					//     if let Err(err) = store.save() {
					//         eprintln!("failed to save store {:?} with error
					// {:?}", store.path, err);     }
					// }
				}
			})
