...
#[tauri::command]
async fn entries<R:Runtime>(app:AppHandle<R>, rid:ResourceId) -> Result<Vec<(String, JsonValue)>> {
	let store = app.resources_table().get::<Store<R>>(rid)?;
...
pub trait StoreExt<R:Runtime> {
	/// Create a store or load an existing store with default settings at the
	/// given path.
	///
	...
	/// 	.setup(|app| {
	/// 		let store =
	/// 			app.store_builder("users.json").auto_save(Duration::from_secs(1)).build()?;
	/// 		Ok(())
	/// 	});
	/// ```
	...
	///
	/// Note that using this function can cause race conditions if you fallback
	/// to creating or loading the store, so you should consider using
	/// [`Self::store`] if you are not sure if the store is loaded or not.
	///
	/// # Examples
	///
	/// ```
