...
pub trait WindowExt {
	...
	#[cfg(feature = "tray-icon")]
	/// Moves the [`Window`] to the given [`Position`] while constraining Tray
	/// Positions to the dimensions of the screen.
	///
...
fn get_monitor_for_tray_icon<R:Runtime>(window:&Window<R>) -> Result<Option<Monitor>> {
	let tray_position =
		window.state::<Tray>().0.lock().unwrap().map(|(pos, _)| pos).unwrap_or_default();
...
/// [`Position`].
fn calculate_position<R:Runtime>(window:&Window<R>, pos:Position) -> Result<PhysicalPosition<i32>> {
	use Position::*;
	...
	let physical_pos = match pos {
		...
		TrayBottomCenter => {
			if let (Some((tray_x, tray_y)), Some((tray_width, _))) = (tray_position, tray_size) {
				PhysicalPosition { x:tray_x + (tray_width / 2) - (window_size.width / 2), y:tray_y }
			} else {
				panic!("Tray position not set");
			}
		},
