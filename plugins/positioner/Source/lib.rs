...
//!
//!   Note: This requires attaching the Tauri plugin, *even* when using the
//! trait extension only.
	...
	let (position, size) = {
		match event {
			...
			| TrayIconEvent::Move { rect, .. } => {
				// tray-icon emits PhysicalSize so the scale factor should not
				// matter.
...
pub fn init<R:Runtime>() -> TauriPlugin<R> {
	let plugin = plugin::Builder::new("positioner").invoke_handler(
